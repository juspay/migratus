...
pub async fn handle_migrate(
    ...
    if crate::cli::commands::customer_global_id::is_customer_global_id_config(&config) {
        return crate::cli::commands::customer_global_id::handle_migrate(
            config_path,
            from_batch,
            count,
            all,
    ...
    let start_batch =
        from_batch.unwrap_or_else(|| get_next_batch_to_migrate(&config.output_config.output_dir));
    ...
    for batch_num in start_batch..=end_batch {
        ...
        match api_client.migrate_batch_with_headers(&batch).await {
            Ok((response, headers)) => {
                ...
                if let crate::operations::api_client::BatchMigrationResponse::Error {
                    status,
                    ref message,
                } = response
                {
                    ...
                    println!(
                        "  migratus migrate {} --from-batch {}",
                        config_path.display(),
                        batch_num
                    );
                    return Err(format!(
                        "Batch {} failed with status {}: {}",
                        batch_num, status, message
                    )
                    .into());
                }
