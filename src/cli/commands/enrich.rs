...
pub async fn handle_enrich(
    ...
    let validated_path = config.output_config.output_dir.join("validated_records.json");
    ...
    println!("📄 Input: {} validated records", validated_output.record_count);
    println!();
    ...
    if config.enrichment.is_none() {
        ...
        let output_path = config.output_config.output_dir.join("enriched_records.json");
        let json = serde_json::to_string_pretty(&output)?;
        fs::write(&output_path, json)?;
        
        println!("💾 Output saved:");
        println!("  → {}", output_path.display());
        println!("  → {} records (no enrichment applied)", output.record_count);
        println!();
