...
use crate::domain::records::{
    FailedMigration, MigrationFailureReason, MigrationMetadata, MigrationResults,
    SuccessfulMigration,
};
use crate::domain::types::{BatchNumber, CustomerId, LineNumber, PaymentMethodId};
use crate::domain::update::{
    FailedUpdate, SuccessfulUpdate, UpdateFailureReason, UpdateMetadata, UpdateResults,
};
...
async fn handle_complete_migrate(
    ...
    for entry in response_files {
        ...
        for result in csv_reader.records() {
            ...
            let unique_key = if let (Some(cust_id), Some(card_masked)) = (
                row_data.get("customer_id"),
                row_data.get("card_number_masked"),
            ) {
        ...
        if let Some(records) = body.as_array() {
            for record_value in records {
                ...
                let unique_key = if let (Some(cust_id), Some(card_masked)) =
                    (&record.customer_id, &record.card_number_masked)
                {
                    format!("{}|{}", cust_id, card_masked)
                } else {
    ...
    if unmatched_count > 0 {
        println!(
            "  ⚠ Warning: {} records could not be matched by unique key",
            unmatched_count
        );
    }
    ...
    println!(
        ...
        migration_results.successful.len()
    );
...
async fn handle_complete_update(
    ...
    if unmatched_count > 0 {
        println!(
            "  ⚠ Warning: {} records could not be matched by payment_method_id",
            unmatched_count
        );
    }
