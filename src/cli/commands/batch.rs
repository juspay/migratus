...
pub async fn handle_batch(
    ...
    let enriched_output: IntermediateOutput<EnrichedRecord> =
        serde_json::from_str(&enriched_json)?;
    
    println!("📄 Input: {} enriched records", enriched_output.record_count);
    println!();
    ...
    println!("  (or: migratus migrate {} --count 10)", config_path.display());
